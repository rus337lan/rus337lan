...
function runSpeedTest() {
    ...
    const downTarget = Math.floor(Math.random() * 400) + 100; // 100–500 Мбит/с
    const upTarget = Math.floor(Math.random() * 200) + 50;    // 50–250 Мбит/с
...
function runScan() {
    ...
    const interval = setInterval(() => {
        if (step >= totalSteps) {
            ...
            const foundCount = Math.floor(Math.random() * 12) + 8; // 8–20 устройств
...
// Установим реальный размер canvas для чёткости
...
// Функция инициализации аудиоконтекста (вызывается при первом воспроизведении)
...
function drawVisualizer() {
    ...
    // Если музыка играет, продолжаем анимацию
    if (isPlaying) {
        animationId = requestAnimationFrame(drawVisualizer);
    } else {
        // Если не играет, оставляем последний кадр
    }
}
...
// Остановка визуализации (можно оставить последний кадр)
...
function playTrack(src) {
    // Инициализируем контекст, если ещё нет
    initAudioContext();
...
// Обработка событий аудио: пауза, окончание
...
// Также если пользователь нажал паузу встроенными контролами — отслеживаем
...
// При загрузке страницы загружаем первый трек (но не играем)
